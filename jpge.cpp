...
void jpeg_encoder::code_block(int component_num)
{
  ...
  if (psnr > 55)
  {
	  for(int i = 0; i < 8; i++)
	  {
		  for(int j = 0; j < 8; j++)
		  {
			  if(m_coefficient_array[8*i + j] > 1 && bitstegokey->get_next_bit_rotate())			
			  {	
	...
	//					printf("%d", bits);
			  ...
			  //printf("%03d ", (unsigned char)m_coefficient_array[8*i + j]);
		  }
		 // printf("\n");
	  }
	//  printf("\n");
...
bool jpeg_encoder::init(output_stream *pStream, int width, int height, int src_channels, const stegosystem &st_params, const params &comp_params)
{
  deinit();
  if (((!pStream) || (width < 1) || (height < 1)) || ((src_channels != 1) && (src_channels != 3) && (src_channels != 4)) || (!comp_params.check())) return false;
  m_pStream = pStream;
  m_params = comp_params;
  m_stparams = st_params;

  bitr = 0;
  bitstr = new bit_stream((void *)m_stparams.stego_data.c_str(), m_stparams.stego_data.length());
...
bool compress_image_to_jpeg_file(const char *pFilename, int width, int height, 
								 int num_channels, const uint8 *pImage_data, const stegosystem &st_params, const params &comp_params)
{	
  ...
  if (!dst_image.init(&dst_stream, width, height, num_channels, st_params, comp_params))
    return false;
...
bool compress_image_to_jpeg_file_in_memory(void *pDstBuf, int &buf_size, int width, int height, int num_channels, const uint8 *pImage_data, const stegosystem &st_params, const params &comp_params)
{
   if ((!pDstBuf) || (!buf_size))
      return false;
